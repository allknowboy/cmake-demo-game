...
void LoginScene::Awake()
{
    ...
    go->set_layer(0x01);
    transform_tree_pot_ = dynamic_cast<Transform*>(go->AddComponent("Transform"));
    ...
    transform_camera_2_->set_position(glm::vec3(5, 0, 10));
    //挂上 Camera 组件
...
void LoginScene::Update()
{
    ...
    //设置相机2
    camera_2_->SetView(glm::vec3(0.1, 0, 0), glm::vec3(0, 1, 0));
    camera_2_->SetProjection(60.f, Screen::aspect_ratio(), 1.f, 1000.f);


    transform_tree_pot_->set_scale(glm::vec3(0.001f, 0.001f, 0.001f));
    transform_tree_pot_->set_position(glm::vec3(0, 0.01, 0));

    // 旋转物体
    if(Input::GetKeyDown(KEY_CODE_R)){
        //旋转物体
        glm::vec3 rotation = transform_tree_pot_->rotation();
        rotation.y += 0.5f;
        rotation.x -= 0.5f;
        transform_tree_pot_->set_rotation(rotation);
    }

    //旋转相机
    if(Input::GetKeyDown(KEY_CODE_LEFT_ALT) && Input::GetMouseButtonDown(MOUSE_BUTTON_LEFT)){
        float degreesX = Input::mousePosition().x_ - last_frame_mouse_position_.x_;
        float degreesY = Input::mousePosition().y_ - last_frame_mouse_position_.y_;
